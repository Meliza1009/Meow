...
export async function makeResultCard(record: GameRecord, before?: string | null, after?: string | null): Promise<Blob> {
  ...
  ctx.fillStyle = '#171A18'; ctx.fillRect(0, 0, canvas.width, canvas.height);
  ctx.strokeStyle = '#F2C94C'; ctx.lineWidth = 3; ctx.strokeRect(42, 42, 996, 1266);
  ctx.fillStyle = '#F2C94C'; ctx.font = '600 34px ui-monospace, monospace';
  ctx.fillText('PHYSICAL ZIP / HUMAN ARCHIVE', 82, 110);
  ctx.fillStyle = '#f0f5f1'; ctx.font = '800 180px ui-monospace, monospace';
  ctx.fillText(`${record.compressedPct}%`, 72, 330);
  ctx.fillStyle = '#F2C94C'; ctx.font = '500 36px ui-monospace, monospace';
  ...
  if (before && after) {
    ...
    ctx.fillStyle = '#303529'; ctx.fillRect(82, 480, 916, 520);
    ctx.strokeStyle = '#F2C94C'; ctx.setLineDash([18, 18]); ctx.strokeRect(112, 510, 856, 460);
    ctx.setLineDash([]); ctx.fillStyle = '#F2C94C'; ctx.font = '500 40px ui-monospace, monospace';
  ...
  ctx.fillStyle = '#abb1a8'; ctx.font = '400 25px ui-monospace, monospace';
